...
conn1 = sqlite3.connect('Expense Data.xbm')
c1 = conn1.cursor()
...
class MainPageGui:
    ...
    def _init_(self, master):
        ...
        #master.iconbitmap('Expense Data.ico')
        ...
        self.credit = tk.Label(master, text = 'Created By : Team expense tracker', fg = 'dark green').grid(row = 3 ,column = 0 , sticky = 'we')
...
class Add_expense:
    def _init_(self , master):
        ...
        self.save = ttk.Button(master , text = 'save' , command = Save_Info).grid(row = 11 , column = 1 , sticky = 'we')
...
def total_debt():
    ...
    total_paid  = c1.execute("SELECT amount_paid  FROM EXPENSES1")
    #placeholders
    combined_paid = 0
    combined_debt = 0
    ...
    total_owed = c1.execute("SELECT Amount_Due  FROM EXPENSES1") 
...
def paid_combiner():
    ...
    total = c1.execute("SELECT amount_paid  FROM EXPENSES1")
    for row in total:
...
def category_searcher(category):
    ...
    c1.execute("SELECT * FROM EXPENSES1 WHERE " +str(category) + "= ?" ,  (str(searchentry.get()),))
    ...
    if c1.fetchall() == []:
        ...
        #insertion of data
        for row in  c1.execute("SELECT * FROM EXPENSES1 WHERE " +str(category) + "= ?" ,  (str(searchentry.get()),)):
            numb_of_row +=1
...
def advanced_Statistics_algo():
    ...
    if searchvar.get() == 'account number':
        ...
        category_searcher('FIRST_NAME')
    elif searchvar.get() ==  'last name':
        category_searcher('LAST_NAME')
    else: 
...
def expense_inserter():
    ...
    info = c1.execute("SELECT * FROM EXPENSES1")
    number_of_row = 0
...
def update_expenses():
    ...
    expense_inserter()
def remove_algo():
    info  = c1.execute("""SELECT * FROM EXPENSES1 WHERE ID_NUM= ?""" ,  (mainentry.get(),))
    #checking to see if the Expense is in the database

    if mainentry.get() == 'ALL':
        c1.execute("""DELETE FROM EXPENSES1""")
        conn1.commit()
        ...
        c1.execute("""DELETE  FROM EXPENSES1 WHERE ID_NUM= ? """ , (mainentry.get(),))
        conn1.commit()
...
def Save_Info():
    ...
    #storing data into database
        ...
        c1.execute("INSERT INTO EXPENSES1 VALUES (?,?,?,?,?,?,?,?)", 
